...
- ✅ **Чекпоинт 3:** Основная бизнес-логика
  - Жизненный цикл опросов (draft → published → closed)
...
- `GET /api/surveys/:surveyId/analytics` — Аналитика
...
- [ ] Swagger / OpenAPI документация (чекпоинт 5)
- [ ] Минимум 5 автотестов (чекпоинт 5)
- [ ] Docker — проект запускается через `docker-compose up` (чекпоинт 5)
